...
class open_api(QAxWidget):
    ...
    def variable_setting(self):
        ...
        self.cf = cf #cf파일안에 있는 값들을 다 가져올 수 있게 해줌. self.cf.password.이렇게
        ...
        if self.account_number == cf.real_account:  # 실전
            ...
            self.db_name_setting(cf.imi1_db_name) #디비네임 만들어주고
            self.mod_gubun = 1
    ...
    def account_info(self):
        ...
        self.account_number = account_number.split(';')[0] #계좌번호를 담았ㄴ다
    ...
    def buy_check(self):
        ...
        if rows != self.today: #today_Buy_stop 에 오늘날짜가 있는지?
    ...
    def buy_num_count(self, invest_unit, present_price):
        ...
        return int(invest_unit / present_price) #투자금액/현재가 = 살수있는 개수

    # 매수 함수
    def trade(self):
        logger.debug("trade 함수에 들어왔다!")
        logger.debug("매수 대상 종목 코드! " + self.get_today_buy_list_code) #매수할 종목의 코드

        # 실시간 현재가(close) 가져오는 함수
        # close는 종가 이지만, 현재 시점의 종가를 가져오기 때문에 현재가를 가져온다.
        current_price = self.get_one_day_option_data(self.get_today_buy_list_code, self.today, 'close') #값을 조회해오는 함수, 구매 목록에 담긴 종가(현재가)를 가져오는 것
        ...
        # 매수 가격 최저 범위
        min_buy_limit = int(self.get_today_buy_list_close) * self.sf.invest_min_limit_rate #현재 매수할 종목의 어제 종가, 설정한 매수가격 범위 (종가보다 2% 크다던지)
        # 매수 가격 최고 범위
        max_buy_limit = int(self.get_today_buy_list_close) * self.sf.invest_limit_rate
        # 현재가가 매수 가격 최저 범위와 매수 가격 최고 범위 안에 들어와 있다면 매수 한다. #장 시작시 급락, 급등을 고려해서
        if min_buy_limit < current_price < max_buy_limit:
            buy_num = self.buy_num_count(self.invest_unit, int(current_price)) #invest_unit : 우리가 종목당 사려고했던 금액
            logger.debug(
            ...
            if not self.jango_check() and self.sf.only_nine_buy: #매수한 다음에 잔고가 있는지 확인
                logger.debug("하나 샀더니 잔고가 부족해진 구간!!!!!")
                # setting_data에 today_buy_stop을 1 로 설정
                self.buy_check_stop() #멈추는 칼럼에 오늘 날짜 입력해줌
        else:
            logger.debug(
    ...
    def get_today_buy_list(self):
        ...
        for i in range(self.sf.len_df_realtime_daily_buy_list):
            ...
            # 종가를 가져온다
            close = self.sf.df_realtime_daily_buy_list.loc[i, 'close'] #사실 현재가를 가져온다는 의미 (현재 시간의 종가니까)
            ...
            if check_item == True: #샀는지 안샀는지 체크하는게 체크 아이템
                continue
            else:
                ...
                self.get_today_buy_list_code = code #위에서 정의한 code
                self.get_today_buy_list_close = close #어제 종가
                # 매수 하기 전에 해당 종목의 check_item을 1로 변경. 즉, 이미 매수 했으니까 다시 매수 하지말라고 체크 하는 로직
                sql = "UPDATE realtime_daily_buy_list SET check_item='%s' WHERE code='%s'" #%s = 1 밑에 정의함
