...
class simulator_func_mysql:
    ...
    def variable_setting(self):
        ...
        # 아래 3개는 분별시뮬레이션 옵션
        # (use_min, only_nine_buy 변수만 각각의 알고리즘에 붙여 넣기 해서 사용) #밑에 시뮬레이션 넘버에 메서드 밑에 붙여넣어서 사용
    ...
    def auto_trade_stock_realtime(self, min_date, date_rows_today, date_rows_yesterday):
        ...
        for j in range(self.len_df_realtime_daily_buy_list):
            if self.jango_check():
                ...
                # 종목 코드를 가져온다.
                code = str(self.df_realtime_daily_buy_list.loc[j, 'code']).rjust(6, "0") #6자리로 만들기 위해서..
    ...
    def get_realtime_daily_buy_list(self):
        ...
        sql = "select * from realtime_daily_buy_list where check_item = '%s' group by code" #싹 다 가져와 살 종목! #check_item 이 0이면 안산거, 날짜가 찍혀있으면 산거

        realtime_daily_buy_list = self.engine_simulator.execute(sql % (0)).fetchall()

        self.df_realtime_daily_buy_list = DataFrame(realtime_daily_buy_list, #매수할 종목들을 이런 형태로 담는다
        ...
        self.len_df_realtime_daily_buy_list = len(self.df_realtime_daily_buy_list) #매수할 종목의 개수
    ...
    def db_to_realtime_daily_buy_list(self, date_rows_today, date_rows_yesterday, i):
        ...
        if len(realtime_daily_buy_list) > 0:
            ...
            if self.op != 'real':
                ...
                # 최종적으로 realtime_daily_buy_list 테이블에 저장 된 종목들을 가져온다. #매수하기 위해서
    ...
    # 테이블의 존재 여부를 파악하는 함수
    def is_simul_table_exist(self, db_name, table_name): #리얼 타임 바이리스트가 있느냐
        ...
        if len(self.min_date_rows) != 0:
            ...
            for t in range(len(self.min_date_rows)):
                ...
                if self.is_simul_table_exist(self.db_name,"all_item_db") and len(self.get_data_from_possessed_item()) != 0:
                    ...
                    if not self.buy_stop and self.jango_check(): #Not False = True
                        # 매수 할 종목을 가져온다
                        self.get_realtime_daily_buy_list()
    ...
    def daily_variable_setting(self):
        self.buy_stop = False #매일 다시 false로 돌려놔야 멈추지않고 9시에 매수하니깐
