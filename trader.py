...
logger.debug("trader start !!!!!!") ##print보다, 언제 어떤 로그가 나왔는지 알려주기 때문에 / 가져온 라이브러리 안에 또 임포트한 라이브러리가 있고 거기에 로거 기능을 임포트해둠
...
class Trader(QMainWindow):
    ...
    def market_time_check(self):
        ...
        self.current_time = QTime.currentTime()
        if self.current_time > self.market_start_time and self.current_time < self.market_end_time: #trader variable_setting에서 정의함
    ...
    def run(self):
        ...
        while 1:
            ...
            if self.market_time_check():
                ...
                if self.open_api.jango_check() and self.buy_time_check() and self.open_api.buy_check(): #돈이 있는지 없는지, 매수설정 시간에 맞는지,
                    # 1,2,3 조건 모두 문제 없으면 매수 시작
                    self.auto_trade_stock()


if __name__ == "__main__":
    app = QApplication(sys.argv) #open api와 연동
    trader = Trader() #트레이더 객체 생성
    trader.run() #런이라는 메서드 호출
